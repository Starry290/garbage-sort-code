...
class Data(Dataset):
    ...
    def load_csv(self,filename):
        ...
        if not os.path.exists(os.path.join(self.root,filename)):
            ...
            print(len(images))
            # 1167 'DataSet\\bulbasaur\\00000000.png'
            # 将文件以上述的格式保存在csv文件内
            random.shuffle(images)
            with open(os.path.join(self.root,filename),mode='w',newline='') as f:
                writer=csv.writer(f)
                for img in images:    #  'DataSet\\bulbasaur\\00000000.png'
        ...
        with open(os.path.join(self.root, filename)) as f:
            reader=csv.reader(f)
            for row in reader:
                # 接下来就会得到 'DataSet\\bulbasaur\\00000000.png' 0 的对象
                img,label=row
                # 将label转码为int类型
                label=int(label)
                images.append(img)
    ...
    def __getitem__(self,idx):
        ...
        # 将图片，label的路径取出来
        # 得到的img是这样的一个类型：'DataSet\\bulbasaur\\00000000.png'
